...
lstm::lstm(int n_neurons, int input_size)
    : n_neurons(n_neurons), input_size(input_size), gen(rd()), dis(0.0, 0.1)
{
...
LSTMOutput lstm::forward(const std::vector<Eigen::VectorXd>& sequence_of_inputs)
{
    ...
    // Initialize gradients here like  
    double init_scale = 0.1;
                   ...
                   const Eigen::VectorXd& dvalues_final)
{
    ...
    for(int t = T - 1; t >= 0; t--)
    {
        ...
        Eigen::VectorXd dtanh2 = tanh_derivative(tanh_ct);
        
        Eigen::VectorXd dhtdtanh = O[t].cwiseProduct(dtanh2);
        
        Eigen::VectorXd dctdft = dhtdtanh.cwiseProduct(ct_prev);
        Eigen::VectorXd dctdit = dhtdtanh.cwiseProduct(C_tilde[t]);
        Eigen::VectorXd dctdct_tilde = dhtdtanh.cwiseProduct(I[t]);
        
        Eigen::VectorXd dtanh1 = dctdct_tilde.cwiseProduct(tanh_derivative(C_tilde[t]));
        Eigen::VectorXd dsigmf = dctdft.cwiseProduct(sigmoid_derivative(F[t]));
        Eigen::VectorXd dsigmi = dctdit.cwiseProduct(sigmoid_derivative(I[t]));
        Eigen::VectorXd dsigmo = dht.cwiseProduct(tanh_ct).cwiseProduct(sigmoid_derivative(O[t]));
        ...
        //Compute gradient for previous timestep 
        if(t > 0) {
